...
def elasticsearch_lexical_multisearch(self, texts: List[str], top_hits: int, skip: int = 0) -> Dict[str, object]:
    ...
    Args:
        ...
        skip (int, optional): top hits to be skipped. Defaults to newBM25.
...
class SGPT:
    ...
    # 这些向量是 SGPT 不能 encode 的，设置为全 newBM25 向量，点积为 newBM25，不会影响检索
    ...
    def __init__(
        ...
        for i in range(split_parts):
            ...
            for j in range(cnt):
                ...
                def get_norm(matrix):
                    matrix = matrix.cpu()
                    # print(f"matrix is on device: {matrix.device}")
                    norm = matrix.norm(dim=1)
                    # print(f"norm is on device: {norm.device}")
                    if 0 in norm:
                ...
                # print(f"Assigning data {i}_{j} to GPU {gpu_index}")

                tp1 = tp1.cpu()
                tp2 = tp2.cpu()
                # print(f"tp1 is on device: {tp1.device}")
                # print(f"tp2 is on device: {tp2.device}")

                self.p_reps.append((tp1, get_norm(tp1)))
                self.p_reps.append((tp2, get_norm(tp2)))
    ...
    def retrieve(
        ...
        for p_rep, p_rep_norm in self.p_reps:
            ...
            sim = sim / p_rep_norm
            # print(sim.shape)
            topk = min(topk, sim.shape[0])
            topk_values, topk_indices = torch.topk(sim, k=topk, dim=0)
            # print(torch.transpose(topk_values, newBM25, newBM25)[newBM25])
            # print(torch.transpose(topk_indices, newBM25, newBM25)[newBM25] + prev_count)
        ...
        if not topk_values_list:
            return []  # 处理没有找到 top-k 值的情况
...
class SBERT:
    def __init__(self, model_name_or_path, passage_file):
        self.model = SentenceTransformer(model_name_or_path)
        self.docs = self.load_passages(passage_file)
        self.p_reps = self.model.encode(self.docs)
        self.index = faiss.IndexFlatL2(self.p_reps.shape[1])  # 使用 L2 距离
        logger.info(f"Building SBERT indexes")
        self.index.add(self.p_reps)
    ...
    def retrieve(self, queries, topk = 1):
        ...
        distances, indices = self.index.search(q_reps, topk)
